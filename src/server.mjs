...
export function createApp({
  suggestAddressesFn = suggestAddresses,
  ...
  app.get("/api/sources", (_request, response) => {
    response.json({ sources: SOURCE_DEFINITIONS, databaseConfigured: hasDatabaseFn() });
  });

  app.get("/api/addresses", rateLimit, async (request, response) => {
    try {
      const q = z.string().trim().min(5).max(180).parse(request.query.q);
      response.json({ addresses: await suggestAddressesFn(q) });
    } catch (error) {
  ...
  app.post("/api/report", rateLimit, async (request, response) => {
    try {
      ...
      response.json(await buildReportFn(input.address));
    } catch (error) {
...
function createRateLimit(buckets) {
